    ...
    public static class LinqTasks
    {
        ...
        public static IEnumerable<Dept> Task15()
        {
            IEnumerable<Dept> result = Emps
                ...
                .Select(g => new Dept
                {
                    Deptno = g.Key ?? 0, // Use the Deptno from Emp, assign 0 if null
                    Dname = g.First().Job, // Use Dname to store Job name (from the first Emp in the group)
                    Loc = g.Count().ToString() // Use Loc to store the count of employees as a string
                });
        ...
        {
            IEnumerable<Dept> result = Emps.Select(e => new Dept
            {
                Deptno = 0,
                Dname = e.Ename,
                Loc = e.Job
